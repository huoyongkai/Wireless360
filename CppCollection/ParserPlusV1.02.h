...
private:
  ...
  //! Find the string \c name
    //! check_vector to check whether is a vector
  std::string findname(const std::string &name,
                       bool &error_flag,
                       bool &print_flag,
                       int num = 0,
                       bool keep_brackets = false,bool check_vector=true);
...
template<class T>
bool ParserPlus::get(T &var, const std::string &name, int num)
{
