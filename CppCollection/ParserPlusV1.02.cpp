...
ParserPlus::ParserPlus(const std::string &filename)
{
    VERBOSE = true;
    init(filename);
}

ParserPlus::ParserPlus(int argc, char *argv[])
{
    VERBOSE = true;
    init(argc, argv);
}

ParserPlus::ParserPlus(const std::string &filename, int argc, char *argv[])
{
    VERBOSE = true;
    init(filename, argc, argv);
}

ParserPlus::ParserPlus(const Array<std::string> &setup)
{
    VERBOSE = true;
    init(setup);
}

void ParserPlus::pre_parsing(void)
{
    int i, j, n, k;
    int count = 0;
    ...
    for (i = 0; i < size; i++) {
        ...
        while (j < n) {
            switch (Line[j]) {
            case '\r'://new problem on Oct 12, 2014, yh3g09
                j++;
                break;
            case '\n':
                //Remove newlines
    ...
    for (i = 0; i < size; i++) {
        ...
        while (j < n) {
            ...
            case '[':
                ...
                if (j == n) {
                    if (count >= TempSetupStrings.size()) {
                        TempSetupStrings.set_size(2*count, true);
                    }
...
void ParserPlus::init(const std::string &filename)
{
    std::string Line;
...
void ParserPlus::init(int argc, char *argv[])
{
...
void ParserPlus::init(const std::string &filename, int argc, char *argv[])
{
    std::string Line;
    int i;
    std::ifstream SetupFile(filename.c_str());
...
void ParserPlus::init(const Array<std::string> &setup)
{
    SetupStrings = setup;
    pre_parsing();
}

void ParserPlus::set_silentmode(bool v)
{
    VERBOSE = !v;
}

bool ParserPlus::exist(const std::string &name)
{
...
template<>
bool ParserPlus::get(std::string &var, const std::string &name, int num)
{
    bool error_flag, print_flag;
...
template<>
bool ParserPlus::get(int &var, const std::string &name, int num)
{
    ivec out;
    bool error_flag, print_flag;
...
template<>
bool ParserPlus::get(bool &var, const std::string &name, int num)
{
    std::string ss;
    bool error_flag, print_flag;
...
bool ParserPlus::get_bool(const std::string &name, int num)
{
    std::string ss;
    bool out = false;
    bool error_flag, print_flag;
    ...
    if (print_flag) {
        cout << "Parsing bool   : " << name << " = " << out << endl;
    }
    return out;
}

int ParserPlus::get_int(const std::string &name, int num)
{
    ivec out;
    bool error_flag, print_flag;
    out = ivec(findname(name, error_flag, print_flag, num));
...
double ParserPlus::get_double(const std::string &name, int num)
{
    double out;
    bool error_flag, print_flag;
    ...
    ss >> out;
    if (error_flag) {
        it_error("Parser: Can not find variable: " + name);
    }
    if (print_flag) {
        cout << "Parsing double: " << name << " = " << out << endl;
    }
    return out;
}

std::string ParserPlus::get_string(const std::string &name, int num)
{
    std::string out;
    bool error_flag, print_flag;
    out = findname(name, error_flag, print_flag, num,true,false);
    if (error_flag) {
        it_error("Parser: Can not find variable: " + name);
    }
    if (print_flag) {
        cout << "Parsing string: " << name << " = " << out << endl;
    }
    return out;
}

vec ParserPlus::get_vec(const std::string &name, int num)
{
    vec out;
    bool error_flag, print_flag;
    ...
    if (print_flag) {
        cout << "Parsing vec   : " << name << " = " << out << endl;
    }
    return out;
}

ivec ParserPlus::get_ivec(const std::string &name, int num)
{
    ivec out;
    bool error_flag, print_flag;
    out = ivec(findname(name, error_flag, print_flag, num));
    ...
    if (print_flag) {
        cout << "Parsing ivec  : " << name << " = " << out << endl;
    }
    return out;
}

svec ParserPlus::get_svec(const std::string &name, int num)
{
    svec out;
    bool error_flag, print_flag;
    ...
    if (print_flag) {
        cout << "Parsing svec  : " << name << " = " << out << endl;
    }
    return out;
}

bvec ParserPlus::get_bvec(const std::string &name, int num)
{
    bvec out;
    bool error_flag, print_flag;
    ...
    if (print_flag) {
        cout << "Parsing bvec  : " << name << " = " << out << endl;
    }
    return out;
}

mat ParserPlus::get_mat(const std::string &name, int num)
{
    mat out;
    bool error_flag, print_flag;
    ...
    if (print_flag) {
        cout << "Parsing mat   : " << name << " = " << out << endl;
    }
    return out;
}

imat ParserPlus::get_imat(const std::string &name, int num)
{
    imat out;
    bool error_flag, print_flag;
    ...
    if (print_flag) {
        cout << "Parsing imat  : " << name << " = " << out << endl;
    }
    return out;
}

smat ParserPlus::get_smat(const std::string &name, int num)
{
    smat out;
    bool error_flag, print_flag;
    ...
    if (print_flag) {
        cout << "Parsing smat  : " << name << " = " << out << endl;
    }
    return out;
}

bmat ParserPlus::get_bmat(const std::string &name, int num)
{
    bmat out;
    bool error_flag, print_flag;
    ...
    if (print_flag) {
        cout << "Parsing bmat  : " << name << " = " << out << endl;
    }
    return out;
}

std::string ParserPlus::findname(const std::string &name, bool &error_flag, bool &print_flag, int num, bool keep_brackets,bool check_vector)
{
    ...
    bool nomoreremove=false;//yh3g09 if a string is detected and the most outside [] or "" are removed, donot remove any [] inside
// cout<<"tmp="<<Temp<<endl;
    n = int(Temp.size());
    Out = "";
    for (i = 0; i < n; i++) {
        ...
        case '[':
            ...
            break;
        case ']':
            if (keep_brackets&&nomoreremove&&i!=n-1) {//yh3g09
                Out += Temp[i];
            }
            if (i == (n - 1)) {
                print_flag = true;
            ...
            break;
        case '"':
            if (keep_brackets) {//yh3g09
                if (nomoreremove&&i!=n-1)
                    Out += Temp[i];
                else if (i==0) //yh3g09
                    nomoreremove=true;
            }
...
//     cout<<"OutFinal="<<Out<<endl;    //Final parsing of vectors and matrices:
