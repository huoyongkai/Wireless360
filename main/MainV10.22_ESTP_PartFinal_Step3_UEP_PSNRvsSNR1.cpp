...
//             centers[i].Z=tmppoint[2];
    ...
    else
    {
        ...
        while ( centers.size() <centers.capacity() )
        {
            ...
            for ( int i=0; i<tmp.size() &&centers.size() <centers.capacity(); i++ )
                centers.push_back ( tmp[i] );
...
//     cout<<"Importance="<<endl<<importance<<endl<<endl;
        for ( int i=0; i<no_frames; i++ )
        {
            string name="Point"+to_str<int> ( i );
            vec tmppoint ( 3 );
            tmppoint[0]=centers[i].X;
            tmppoint[1]=centers[i].Y;
            tmppoint[2]=centers[i].Z;
...
//             data.Write<mat> ( name+"_imp",importance[i] );
        ...
        for ( int i=0; i<centers.size(); i++ )
        {
            ...
            globe2rect.CalImportanceOfBlocks ( halfviewangle,centers[i],delta,importance[i] );
        }
        for ( int i=0; i<no_frames; i++ )
        {
            string name="Point"+to_str<int> ( i );
...
//                         pturbo->Decode ( vecDeinterleaved[vind] ,vecSrc[vind] ,vecDecoded[vind]  );
                    ...
                    //count bit errors for calculation of ber
                    if(frmind==0)
                    {
                        ber_counters(vind)+=BERC::count_errors ( vecSrc(vind),vecDecoded(vind) );
                        bits_counters(vind)+=vecSrc(vind).size();
                    }
                }
...
//                 cout<<"\nstart decode"<<endl;
                ...
                YUVPSNR tmpresult=globe2rect.CalPSNR_YUV_AverN ( halfviewangle,centers, orgYUV,tpFile,"4:2:0",videoparas.Get_Dims(),psnr_zero2one,&frames_psnr );
                cout<<tmpresult<<endl;
                psnr+=tmpresult;
                counter++;
                frmind=0;
        ...
        if ( jobs.FinishJob<int> ( 0,1 ) ==0 )
        {
            ...
            out.open ( "BerResults.txt",ios::out|ios::app );
            out<<snr;
            for(int i=0; i<ber_counters.size(); i++)
                out<<"  "<<ber_counters(i)/bits_counters(i);
            out<<endl;
            out.close();
            mutexer.Unlock();
        }
