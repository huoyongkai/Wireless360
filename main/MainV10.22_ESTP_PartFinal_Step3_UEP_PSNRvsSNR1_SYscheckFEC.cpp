...
// rsc.d
    ...
    int no_view=videoparas.Get_FrmNumber();
    ...
    {
        centres.reserve ( no_view +videoparas.Get_FPS() );
        for ( int sec=0; sec< ( no_view +videoparas.Get_FPS()-1 ) /videoparas.Get_FPS(); sec++ )
        {
...
//     cout<<"Importance="<<endl<<importance<<endl<<endl;
    ...
    {
        jobname="Simfactor_"+itpp::to_str ( simsnrs[ind_tp] ); //  Converter::Double2Str ( throughputs[ind_tp],15 );
...
//         index++;
    ...
    vector<mat> finalratesinv ( no_view );
    vector<bmat> puncmat ( no_view );
...
//         pturbo->ClearDecodingCounter();
        ...
        do
        {
            ...
            if ( uepVR.IsCritical ( types ( 0,0 ) ) )
            {
                ...
                {
                    finalratesinv[frmind]=uepVR.Get_OptimalRates_subframes_matlab ( dist[frmind],bitlens,importance[frmind],snr,overallrate );
                    cout<<"coding rate="<<finalratesinv[frmind] <<endl;
                    for ( int vind=0; vind<finalratesinv[frmind].size(); vind++ )
                    {
                        if ( finalratesinv[frmind] ( vind ) >0 )
                        {
                            finalratesinv[frmind] ( vind ) =1.0/Puncturer::Generate_Puncmat ( puncmat[frmind],iterCfg.get<ivec> ( "Gentor" ).size(),1.0/finalratesinv[frmind] ( vind ) );
                            //      p_lg<<puncmat<<endl;
                            //      cout<<puncmat.rows()<<" "<<puncmat.cols()<<endl;
                            punc[vind].Set_punctureMatrix ( puncmat[frmind] );
                        }
                    }
                }
                frmind++;
...
//                         pturbo->Encode ( vecSrc ( vind ),vecCoded ( vind ) );
                        ...
                        punc[vind].Puncture ( vecCoded ( vind ),puncedcoded ( vind ) );
                        interleaver.Clear();
                        ...
                        punc[vind].Depuncture ( deinterleaved,vecDeinterleaved ( vind ) );
...
//                         pturbo->Decode ( vecDeinterleaved[vind] ,vecSrc[vind] ,vecDecoded[vind]  );
            ...
            {
                p_lg<<endl;
                string tpFile=Replace_C ( "decoded_&.yuv",'&',p_cvt.To_Str<long> ( long ( snr ) ) +"TP" );
...
//                 cout<<"\nstart decode"<<endl;
                ...
                psnr+=RawVideo::CalPSNR_YUV_AverN ( orgYUV,tpFile,"4:2:0",videoparas.Get_Dims(),psnr_zero2one,&frames_psnr );
                cout<<psnr<<endl;
                counter++;
                frmind=0;
            }
